...
static int zip_mkpath(char* path) {
    char* p;
    char npath[MAX_PATH2 + 1];
    int len = 0;
    int has_device = HAS_DEVICE(path);

    memset(npath, 0, MAX_PATH2 + 1);
    if (has_device) {
        // only on windows
        npath[0] = path[0];
        npath[1] = path[1];
    ...
    for (p = path + len; *p && len < MAX_PATH2; p++) {
...
static int zip_archive_extract(mz_zip_archive* zip_archive, const char* dir,
    ...
    int err = 0;
    mz_uint i, n;
    char path[MAX_PATH2 + 1];
    char symlink_to[MAX_PATH2 + 1];
...
#if defined(_MSC_VER)
        strncpy_s(&path[dirlen], MAX_PATH2 - dirlen, info.m_filename,
            MAX_PATH2 - dirlen);
#else
        strncpy(&path[dirlen], info.m_filename, MAX_PATH2 - dirlen);
#endif
        err = zip_mkpath(path);
        if (err < 0) {
            // Cannot make a path
    ...
    defined(__MINGW32__)
#else
            if (info.m_uncomp_size > MAX_PATH2 ||
